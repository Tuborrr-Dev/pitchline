...
import { isPreKickoffFixture } from "./event-formatting";
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center text-center">
        ...
        <motion.div
          key={`${fixture.scoreA}-${fixture.scoreB}`}
          initial={{ scale: 0.96 }}
          animate={{ scale: 1 }}
          transition={{ duration: 0.2 }}
          className="mt-1 border border-[var(--terminal-border)] bg-[var(--terminal-panel)] px-4 py-2 font-display text-[2.25rem] font-bold uppercase leading-none text-[var(--terminal-text-strong)] sm:mt-2 sm:px-8 sm:py-3 sm:text-[4rem]"
        >
        ...
        <div className="mt-2 flex items-center justify-center gap-2">
          ...
          />
          {centerBadgeText && (
            <div className="border border-[var(--terminal-green)] bg-emerald-500/10 px-2 py-0.5 font-mono text-[0.5rem] font-semibold uppercase text-[var(--terminal-green)] sm:px-3 sm:text-[0.72rem]">
              {centerBadgeText}
            </div>
          )}
        </div>
